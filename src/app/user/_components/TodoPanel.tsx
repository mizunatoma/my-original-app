...
export default function TodoPanel({
  ...
  return (
    <aside
      className={`fixed bottom-0 top-0 z-20 overflow-auto bg-[#FCFAF7] transition-all duration-300 ${isCollapsed ? 'left-[80px]' : 'left-[160px]'} ${isTodoPanelOpen ? 'w-[300px] border border-[#EFEDE6] p-4' : 'w-[0px]'}`}
    >
      {/*listチップ一覧*/}
      {list?.todoLists?.map((list) => (
        <button
          key={list.id}
          className={`inline-flex items-center rounded-xl bg-orange-100 px-2 text-sm font-medium text-orange-800 ${list.id === selectedListId && 'border-2 border-orange-200'}`}
      ...
      {/*list追加*/}
      <button
        className="inline-flex items-center rounded-t-lg bg-orange-200 px-3 py-1 text-sm font-medium text-orange-800"
      ...
      {isOpen && (
        <div>
          ...
          />
          <button
            className="rounded-lg border border-[#5A8B7D] p-1 text-center text-sm text-[#5A8B7D] hover:bg-[#F2F0E9]"
            ...
            {editingId !== todo.id ? (
              ...
              <>
                ...
                >
                  <SquarePen
                    size={16}
                    className="text-gray-500 hover:text-orange-200"
                  />
                </button>
                <button
      ...
      {selectedListId !== null && (
        <button
          className="mt-auto flex w-full items-center justify-end gap-1 p-2 text-right text-red-400 hover:text-red-600"
